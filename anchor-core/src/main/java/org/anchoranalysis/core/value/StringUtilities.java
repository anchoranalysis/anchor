...
public class StringUtilities {
    ...
    /**
     * Like {@link String#join} but ignores any empty strings, and returns {@link Optional#empty} if all components are empty.
     * 
     * @param delimeter delimeter to use when joining strings
     * @param stringMaybeEmpty strings that may or may not be empty.
     * @return a joined string including only the non-empty strings, and with a delimeter between them.
     */
    public static Optional<String> joinNonEmpty(String delimeter, String... stringMaybeEmpty) {
        ...
        return OptionalUtilities.createFromFlag(!components.isEmpty(), () -> String.join(delimeter, components));
    }
