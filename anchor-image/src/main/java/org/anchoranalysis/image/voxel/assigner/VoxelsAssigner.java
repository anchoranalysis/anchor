 ...
 * 
 * <p>The co-ordinates of the bounding-box, object-mask etc. are always 
 * <i>relative</i> to a particular coordinate frame.
 * 
 * <p>Throughout the code-base, usually this interface is only exposed
 * expecting <b>global</b> coordinates (i.e. not relative to anything else) for
 * bounding-boxes etc.
 * 
 * @author Owen Feehan
 */
public interface VoxelsAssigner {
    ...
    /**
     * Sets voxels in a box to a particular value if they match an object-mask <b>and</b> every voxel to be set matches a predicate
     * <p>
     * If any one of the voxels in the object doesn't match the predicate, the operation is aborted, and nothing is written at all.
     * 
     * @param object the object-mask to restrict which values in the buffer are assigned
     * @param voxelPredicate the existing value of every voxel to be written must match this predicate, otherwise no voxels are set at all
     * @return the number of voxels successfully "set" or -1 if at least one voxel didn't match the predicate wasn't matched
     */
     ...
     * 
     * @see {#link {@link #toObject(BoundingBox, ObjectMask, Optional)} for a more customizable version
     * @param object the object-mask to restrict where voxels are set
     * @param restrictTo a restriction on where to process in the object-mask (expressed in the same coordinates as {@code object}).
     * @return the number of voxels successfully "set"
     */
     ...
     * @param restrictTo only process this region (which is sensibly part or all of the intersection of the two objects bounding-boxes) 
     * @return the total number of pixels written
     * @throws OperationFailedException if {@code restrictTo} does not intersect
     */
    int toEitherTwoObjects(
            ObjectMask object1,
            ObjectMask object2,
