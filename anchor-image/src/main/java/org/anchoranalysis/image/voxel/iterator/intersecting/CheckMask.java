...
/**
 * Processes voxels checking that they lie on a mask, converting a {@link ProcessBufferTernary} to a {@code ProcessBufferBinary<UnsignedByteBuffer>}.
 *  
 * @author Owen Feehan
 *
 */
@AllArgsConstructor class CheckMask implements ProcessBufferTernary<UnsignedByteBuffer> {

    private final ProcessBufferBinary<UnsignedByteBuffer> process;
    private final byte onMaskGlobal;
    
    @Override
    public void process(Point3i point, UnsignedByteBuffer buffer1, UnsignedByteBuffer buffer2,
            UnsignedByteBuffer bufferObject, int offset1, int offset2, int offsetObject) {
