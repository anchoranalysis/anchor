...
/**
 * Iterates over {@link Voxels} using the (slower) {@code getInt} and {@code putInt} methods of {@link VoxelBuffer} rather than directly accessing the buffers
 *
 * @author Owen Feehan
 */
...
public class IterateVoxelsAsInt {
    ...
    public static void changeEachPoint(Voxels<?> voxels, IntUnaryOperator operator) {
        ...
        voxels.slices().iterateOverSlices( buffer-> {
    ...
    /**
     * Assigns a value to any voxel matches a predicate, reading and writing the buffer as an {@code int}.
     * <p>
     * Note this provides slower access than operating on the native-types.
     *  
     * @param voxels the voxels, each of which is tested by {@code predicate} and maybe assigned a new value
     ...
     * @param object the object-mask (global coordinates) to restrict which voxels are tested with the predicate
     ...
     * <p>
     * {@code boxVoxels} and {@code boxRelativeToObject} must both have the same extent.
     * 
     * @param <T> buffer-type
    ...
    public static <T extends Buffer> int callEachPoint(
        ...
        Optional<BoundingBox> restrictToIntersection = OptionalUtilities.flatMap( restrictTo,
                box -> box.intersection().with(object.boundingBox()) );
        
        if (restrictTo.isPresent() && !restrictToIntersection.isPresent()) {
            // There's no intersection between the object-mask and restrictTo, so there's nothing to be done. Exit early
            return 0;
        }
        
        // What part of the object-mask is iterated over. If not restricted, all of the mask is iterated over */
        BoundingBox iterateBox = restrictToIntersection.map( box -> box.relativePositionToBox(object.boundingBox()) ).orElseGet(boxVoxels::shiftToOrigin);
     ...
     * @param object the object-mask (global coordinates)
     * @param boxRelativeToObject bounding-box expressed <i>relative</i> to the object-mask's bounding-box
     * @param process processes each point that fulfills the conditions
     * @return the total number of points processed (0 if none are)
     */
    private static <T extends Buffer> int callEachPoint(Voxels<T> voxels, BoundingBox boxVoxels, ObjectMask object, BoundingBox boxRelativeToObject, ProcessVoxelSlice<T> process) {    
