...
public class IterateVoxelsRemaining {
    ...
    /**
     * Iterate over remaining voxels each voxel - with <b>two</b> associated <b>buffers</b> for
     * each slice.
     * 
     * <p>Precondition: that both buffer's current position are jointly set to a position, with
     * an identical number of remaining voxels, otherwise an exception is thrown e.g. two identically sized buffers with offset=0. 
     *
    ...
    public static <S,T> void withTwoBuffersWithoutOffset(
            VoxelBuffer<S> voxelBuffer1, VoxelBuffer<T> voxelBuffer2, ProcessBufferBinaryWithoutOffset<S,T> process) {
