...
class ScaledLabelledObjects {
    ...
    public ScaledLabelledObjects(ObjectsWithBoundingBox objectsWithBox, Optional<UnaryOperator<ObjectMask>> preOperation, ScaleFactor scaleFactor) throws CreateException {
    ...
    public Map<ObjectMask,ObjectMask> buildMapOfAllScaledObjects(Optional<UnaryOperator<ObjectMask>> postOperation) throws CreateException {
                
        UnaryOperator<ObjectMask> postOperationWithShift = object -> shiftByAndMaybePost(object, cornerScaled, postOperation);
        
        // Read the objects from the raster Ids, and create a corresponding scaled object
        Map<ObjectMask,ObjectMask> map = new CreateObjectsFromLabels(labelsScaled.voxels().any(), 1, numberLabels).create(labelMapping, postOperationWithShift);
        
        // Scaling the overlapping objects
        objectsOverlap.stream().forEach( overlappingObject-> scaleObjectIndependently(overlappingObject, labelsScaled.dimensions().extent(), scaleFactor, map, postOperationWithShift) );
        
        return map;
    }

    /** Creates a channel with labels for the unscaled (non-overlapping objects) and places overlapping objects in {@code objectsOverlap} */
    private Channel createLabels(ObjectsWithBoundingBox objectsWithBox, Optional<UnaryOperator<ObjectMask>> preOperation) throws CreateException {
        LabelObjects labeller = new LabelObjects(preOperation, Optional.of(objectsOverlap::add));
        return labeller.createLabelledChannelForObjects(
              objectsWithBox,
              Optional.of(labelMapping)
        );
    }
     ...
     */
    private ObjectMask shiftByAndMaybePost(ObjectMask object, ReadableTuple3i shift, Optional<UnaryOperator<ObjectMask>> postOperation) {
    ...
    private static void scaleObjectIndependently(OverlappingObject unscaled, Extent extent, ScaleFactor scaleFactor, Map<ObjectMask,ObjectMask> map, UnaryOperator<ObjectMask> postOp) {
    ...
    /**
     * We use a nearest neighbour interpolator as we want only distinct labels as an output, therefore no combining of intensity values
     * 
