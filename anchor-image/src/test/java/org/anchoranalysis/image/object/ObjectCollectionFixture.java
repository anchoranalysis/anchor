...
public class ObjectCollectionFixture {
    ...
    public ObjectCollectionFixture(int numberNonOverlappingObjects, int numberOverlappingObjects, int distanceBetweenShift, boolean incrementY) {
    ...
    public Extent extentLargerThanAllObjects() {
        int heightAllObjects = incrementY ? numberObjects * (HEIGHT + distanceBetweenShift) : HEIGHT;
        int depthAllObjects = do3D ? DEPTH : 1;
        return new Extent(
            numberObjects * (WIDTH +  distanceBetweenShift),
            heightAllObjects,
            depthAllObjects);
    }
