...
public class FindOutlineTest {
    ...
    private static int expectedNumberVoxelsEntire(boolean use3D) {
        return RectangleObjectFixture.RECTANGLE_WIDTH * RectangleObjectFixture.RECTANGLE_HEIGHT * (use3D ? RectangleObjectFixture.RECTANGLE_DEPTH : 1);
    }
    ...
    private static int expectedNumberVoxelsObject(boolean use3D, int outlineSize) {
        if (use3D) {
            if (outlineSize==1) {
                ...
                throw new AnchorFriendlyRuntimeException("If use3D==true, then only outlineSize==1 is supported in the tests");
            }
        } else {
    ...
    private static int numberVoxelsOutline3D() {
        ...
        int partialSlice =  numberVoxelsOutlineSlice(RectangleObjectFixture.RECTANGLE_WIDTH, RectangleObjectFixture.RECTANGLE_HEIGHT);
        int fullSlice = RectangleObjectFixture.RECTANGLE_WIDTH * RectangleObjectFixture.RECTANGLE_HEIGHT;
    ...
    private static int numberVoxelsOutline2D(int outlineSize) {
        ...
        for( int i=0; i<outlineSize; i++ ) {
            int subtract = i*2;
            sum += numberVoxelsOutlineSlice(RectangleObjectFixture.RECTANGLE_WIDTH - subtract, RectangleObjectFixture.RECTANGLE_HEIGHT - subtract);
        }
        return sum;
    }
    ...
    private static void assertNumberVoxels(String messagePrefix, int expectedNumberVoxels, ObjectMask object) {
        assertEquals(messagePrefix + " - number of voxels", expectedNumberVoxels, object.numberVoxelsOn());
    }
}
