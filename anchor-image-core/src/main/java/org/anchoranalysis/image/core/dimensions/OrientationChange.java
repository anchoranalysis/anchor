 ...
 * 
 * <p>It is also possible for a mirroring to occur (X coordinates flipped in direction, but not in the Y dimension)
 * for each respective orientation change.
 *
 * @author Owen Feehan
 */
public enum OrientationChange {
    ...
    /**
     * Pixels should appear 90 degrees rotated in the anticlockwise direction, <b>and then</b> mirrored across the X-dimension.
     */
    ...
    /** Pixels should appear 180 degrees rotated in the clockwise direction, <b>and then</b> mirrored across the X-dimension. */
    ROTATE_180_MIRROR {
        @Override
        public int index(int existingIndex, int x, int y, Extent extent) {
            return index(x, y, extent);
    ...
    /**
     * Pixels should appear 90 degrees rotated clockwise, <b>and then</b> mirrored across the X-dimension.
     */
     ...
     * @return either a newly created {@link Extent} if it is changed, or else {@code extent} if no change is needed.
     */
     ...
     * @return either a newly created {@link Resolution} if it is changed, or else {@code resolution} if no change is needed.
     */
     ...
     * @return either a newly created {@link Dimensions} if it is changed, or else {@code dimensions} if no change is needed.
     */
