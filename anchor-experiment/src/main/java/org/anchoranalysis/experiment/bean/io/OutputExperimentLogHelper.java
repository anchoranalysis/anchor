...
/** Helps creating and outputting messages to the log for {@link org.anchoranalysis.experiment.bean.io.OutputExperiment} */
...
class OutputExperimentLogHelper {
	...
	public static void maybeLogStart(ParametersExperiment params) {
		if (params.isDetailedLogging()) {
			params.getLoggerExperiment().logFormatted(
	...
	public static void maybeLogCompleted(ParametersExperiment params, StopWatch stopWatchExperiment) {
		if (params.isDetailedLogging()) {
			params.getLoggerExperiment().logFormatted(
